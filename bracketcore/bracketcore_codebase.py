...
class BracketModel:
    ...
    def calculate(self, sg: Seeding, tc: TeamContainer, sc: SeriesContainer, df: Differentials) -> None:
        ...
        for k, v in self._matchups.items():
            ...
            if (self.results[k].winner if isinstance(self.results[k], Matchup.Result) else self.results[k].winners) is None:
                self.complete = False


tc = TeamContainer()
tc.register(Team(0, "GZ"))
tc.register(Team(1, "DIG"))
tc.register(Team(2, "BDS"))
tc.register(Team(3, "NRG"))
tc.register(Team(4, "G2"))
tc.register(Team(5, "END"))
tc.register(Team(6, "FAZE"))
tc.register(Team(7, "SMPR"))
tc.register(Team(8, "NV"))
tc.register(Team(9, "VE"))
tc.register(Team(10, "COL"))
tc.register(Team(11, "ERA"))
tc.register(Team(12, "SRG"))
tc.register(Team(13, "RNG"))
tc.register(Team(14, "VIT"))
tc.register(Team(15, "FUR"))


sc = SeriesContainer(tc)
# sc.register(Series(tc[""], tc[""], 000, 000))
sc.register(Series(tc["GZ"], tc["NV"], 2, 3))
sc.register(Series(tc["DIG"], tc["VE"], 3, 0))
sc.register(Series(tc["BDS"], tc["COL"], 3, 2))
sc.register(Series(tc["NRG"], tc["ERA"], 3, 1))
sc.register(Series(tc["G2"], tc["SRG"], 0, 3))
sc.register(Series(tc["END"], tc["RNG"], 1, 3))
sc.register(Series(tc["FAZE"], tc["VIT"], 3, 1))
sc.register(Series(tc["SMPR"], tc["FUR"], 3, 2))
sc.register(Series(tc["DIG"], tc["NV"], 3, 0))
sc.register(Series(tc["SRG"], tc["SMPR"], 1, 3))
sc.register(Series(tc["NRG"], tc["BDS"], 1, 3))
sc.register(Series(tc["FAZE"], tc["RNG"], 3, 1))
sc.register(Series(tc["END"], tc["VIT"], 3, 1))
sc.register(Series(tc["COL"], tc["ERA"], 3, 1))
sc.register(Series(tc["FUR"], tc["G2"], 3, 2))
sc.register(Series(tc["GZ"], tc["VE"], 3, 0))


df = Differentials(tc)


initial_sg = Seeding(tc)
initial_sg.set("GZ", "DIG", "BDS", "NRG", "G2", "END", "FAZE", "SMPR", "FUR", "VIT", "RNG", "SRG", "ERA", "COL", "VE", "NV")
# initial_sg.set("GZ", "DIG", "BDS", "NRG", "G2", "END", "FAZE", "SMPR", "NV", "VE", "COL", "ERA", "SRG", "RNG", "VIT", "FUR")

def swiss() -> BracketModel:
    BR = BracketModel()
    seed_sort = (
        (-1, lambda: BR.df.rgd),
        (1, lambda: BR.sg)
    ...
    BR["r2_h"] = MatchSet(
        lambda: BR["r1"].result_.winners.sort(
            *seed_sort
        ),
        SeedingInterpreter.reversed
    )
    BR["r2_l"] = MatchSet(
        lambda: BR["r1"].result_.losers.sort(
            *seed_sort
        ),
        SeedingInterpreter.reversed
    )
    BR["r3_h"] = MatchSet(
        lambda: BR["r2_h"].result_.winners.sort_no_rematches(
            sc,
            SeedingInterpreter.reversed,
            *seed_sort
        ),
    ...
    BR["r3_m"] = MatchSet(
        lambda: Seeding(tc).set(
            *BR["r2_h"].result_.losers.sort(
                *seed_sort
            ).seeding_,
            *BR["r2_l"].result_.winners.sort(
                *seed_sort
            ).seeding_
        ).sort_no_rematches(
            sc,
...
BR = single_elim_4_team()
BR.calculate(Seeding(tc).set("GZ", "DIG", "VE", "NV"), tc, sc, df)
print(BR.complete)




# print([(result.team_1.name if result.team_1 is not None else None, result.team_2.name if result.team_2 is not None else None) for result in BR.results["r3_m"].results])
# # print([(team.name, df.rgd[team]) for team in BR.results["r1"].losers.sort(*seed_sort).seeding_])
# # print(df.rgd)
# def sort(teams):
#     _sorted = [[i, GD[i], s1r1_seeding.index(i)] for i in teams]
#     _sorted.sort(key = lambda x: (-x[1], x[2]))
#     _sorted = [i[0] for i in _sorted]
#     for i in [list(range(int(len(teams)/2)))+list(i) for i in list(reversed(list(itertools.permutations(range(len(teams)-1,int(len(teams)/2)-1,-1),int(len(teams)/2)))))]:
#         _teams = [_sorted[j] for j in i]
#         rematch = False
#         for k in range(int(len(_sorted) / 2)):
#             __teams = [_teams[k],_teams[len(_sorted)-(k+1)]]
#             __teams.sort()
#             if "_".join(__teams) in MATCHUPS_PLAYED:
#                 rematch = True
#                 break
#         if rematch:
#             break
...
# B = LinearBracketBuilder(sc, df)
# B.add_matchup("r1s1", Matchup(tc[0], tc[1]))
# B.add_matchup("r1s2", Matchup(tc[2], tc[3]))
# B.add_matchup("r2s1", Matchup(B["r1s1"].winner, B["r1s2"].winner))


# B.calculate()
# print(B.results["r2s1"].team_1)


# sg2 = Seeding(tc)
# sg2.set(4, 6, 7, 5, 0, 1, 2, 3)
# sg = Seeding(tc)
# sg.set(0, 1, 2, 3, 4, 5, 6, 7)
# # print(id(tc[0]))
# # print(sg.seeding)
# # print(sg.seeding.index(tc[0]))
# sg.sort(
#     (-1, df.rgd),
#     (-1, df.rsd),
#     (1, sg2)
...
# sg = Seeding(tc)
# sg.set_seeding(0, 1, 2, 3, 4, 5, 6, 7)


# print(SeedingInterpreter.random(sg))


# df = Differentials(tc)

# sc = SeriesContainer(tc)
# sc.register(Series(tc[0], tc[1], 3, 1))


# matchup = Matchup(tc[0], tc[1])
# getter  = matchup.result
# print(getter().is_winner_1)
# matchup.calculate(sc, df)
# print(getter().is_winner_1)
# print(df.rgd)
# df.rgd.reset()
# print(df.rgd)
...
# sc = SeriesContainer(tc)
# sc.register(Series(tc[0], tc[1], 3, 2))
# sc.register(Series(tc[2], tc[3], 1, 3))
# sc.register(Series(tc[4], tc[5], 1, 3))
# sc.register(Series(tc[6], tc[7], 2, 3))
# sc.register(Series(tc[0], tc[3], 3, 1))

# sg = Seeding(tc)
# sg.set_seeding(0, 1, 2, 3, 4, 5, 6, 7)


# def double_elim_8_team(sc: SeriesContainer, sg: Seeding) -> LinearBracketBuilder:
#     b = LinearBracketBuilder(sc)
#     # Upper round 1
#     b.add_matchup("u_r1_s1", b.Matchup(sg.seeding[0], sg.seeding[1]))
#     b.add_matchup(2, b.Matchup(sg.seeding[2], sg.seeding[3]))
#     b.add_matchup(3, b.Matchup(sg.seeding[4], sg.seeding[5]))
#     b.add_matchup(4, b.Matchup(sg.seeding[6], sg.seeding[7]))
#     # Lower round 1
#     b.add_matchup(5, b.Matchup(b["u_r1_s1"].loser, b[2].loser))
#     b.add_matchup(6, b.Matchup(b[3].loser, b[4].loser))
#     # Upper quarterfinals
#     b.add_matchup(7, b.Matchup(b["u_r1_s1"].winner, b[2].winner))
#     b.add_matchup(8, b.Matchup(b[3].winner, b[4].winner))
#     # Lower round 2
#     b.add_matchup(9, b.Matchup(b[7].loser, b[5].winner))
#     b.add_matchup(10, b.Matchup(b[8].loser, b[6].winner))
#     # Lower quarterfinals
#     b.add_matchup(11, b.Matchup(b[9].winner, b[10].winner))
#     # Upper semifinals
#     b.add_matchup(12, b.Matchup(b[7].winner, b[8].winner))
#     # Lower semifinals
#     b.add_matchup(13, b.Matchup(b[12].loser, b[11].winner))
#     # Finals
#     b.add_matchup(14, b.Matchup(b[12].winner, b[13].winner))
#     return b

# match_schematic = double_elim_8_team(sc, sg)
# match_schematic.calculate()
# for k, v in match_schematic.results.items():
#     print(k, v.t1.name if v.t1 else None, v.s1, v.s2, v.t2.name if v.t2 else None)

# print(match_schematic.complete)
